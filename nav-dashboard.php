<div class="contain-to-grid page-nav-warpper">

	<nav class="top-bar page-nav" data-topbar>
		<div class="row">

			<li class="columns medium-4"><a href="<?php the_permalink();?>">Dashboard</a></li>
			<li class="columns medium-2"><a href="<?php the_permalink();?>">1. Calculators</a></li>
			<li class="columns medium-2"><a href="<?php the_permalink();?>">2. Reivew</a></li>
			<li class="columns medium-2"><a href="<?php the_permalink();?>">3. Cart</a></li>
			<li class="columns medium-2"><a href="<?php the_permalink();?>">4. Checkout</a></li>

			<div class="progress">
				<span class="meter" style="width: 0%"></span>
			</div>
		</div>

	</nav>

</div>	